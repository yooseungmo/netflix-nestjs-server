...
describe('AuthController', () => {
  ...
  describe('registerUser', () => {
    it('should register a user', () => {
      ...
      const result = { id: 1, email: 'testBlanc@gmail.com' };
  ...
  describe('blockToken', () => {
    it('should block a token', async () => {
      ...
      jest.spyOn(authService, 'tokenBlock').mockResolvedValue(result);

      expect(authController.blockToken(token)).resolves.toEqual(result);
  ...
  describe('loginUserPassport', () => {
    it('should login user using passport strategy', async () => {
      ...
      const req = { user };
      const refreshToken = 'mocked.access.token';
      const accessToken = 'mocked.refresh.token';
